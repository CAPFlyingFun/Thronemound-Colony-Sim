...
function floorWall() {
  ...
  const walker = new SurfaceWalker(density, {
    ...
    gripReach: 9 / MM,
    align: 12,
    maxTiltRate: (Number(process.env.TILT_CAP ?? 240) * Math.PI) / 180,
    tiltAccel: (Number(process.env.TILT_ACC ?? 2400) * Math.PI) / 180,
    goalGain: 1000,
    snap: 14,
  deadband: 0.06,
    gravity: 9,
...
describe('continuous corner body handoff', () => {
  ...
  it('starts curving toward the wall after two grips before the root penetrates it', () => {
    ...
    console.log(`MARGIN cap=${process.env.TILT_CAP ?? 240} twoAt=${two} guidedAt=${guided?.frame} `
      + `wallPenMm=${guided?.wallPenMm.toFixed(4)} framesFromTwoTo8deg=${(guided?.frame ?? -1) - two}`);
