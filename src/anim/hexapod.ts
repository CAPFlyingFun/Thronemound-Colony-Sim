...
export interface RigMap {
  ...
  body: string[];
  thorax: string[];
  /** The mouthpart stub. The auto-rigger left the mandibles out. */
  mouth: string[];
  antennaLeft: string[];
  antennaRight: string[];
  gaster: string[];
 ...
 */
export function gaitPose(input: GaitInput): GaitPose {
  ...
  rotations.set(QUEEN_RIG.antennaLeft[0]!, [
    Math.sin(clock * 3.1) * sweep * 0.6,
    Math.sin(clock * 2.3) * sweep,
    0,
  ]);
  rotations.set(QUEEN_RIG.antennaRight[0]!, [
...
/**
 * Real-world size, so she can be scaled honestly rather than to taste.
 *
 * A mated Solenopsis invicta queen runs about 9 mm nose to gaster; workers are
 * 3 to 6. One voxel is 5 mm, so she is a little under two cubes long — which
 * is why a one-cube tunnel will not take her and she has to widen her own
 * galleries. Stated as millimetres and divided here so the day the voxel scale
 * changes, this follows it.
 */
export const QUEEN_LENGTH_MM = 9;
export const VOXEL_MM = 5;
/** The model measures this along Z in its own units, feet on the ground. */
export const MODEL_LENGTH_UNITS = 4.49;

export function queenScale(): number {
  return QUEEN_LENGTH_MM / VOXEL_MM / MODEL_LENGTH_UNITS;
}
